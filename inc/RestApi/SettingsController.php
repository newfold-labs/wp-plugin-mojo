...
class SettingsController extends \WP_REST_Controller {
			...
			if ( ! is_null( $params[ $setting ] ) ) {
				...
				switch ( $setting ) {
					...
					case 'autoUpdatesPlugins':
						...
						if ( $new_value ) {
							\MOJO\sync_plugin_update_settings();
						}
					...
					case 'autoUpdatesThemes':
						// Keep the WordPress Core setting in sync.
						if ( $new_value ) {
							\MOJO\sync_theme_update_settings();
						}
	...
	public function check_permission() {
		if ( ! current_user_can( 'manage_options' ) ) {
			return new \WP_Error( 'rest_forbidden_context', __( 'Sorry, you are not allowed to access this endpoint.', 'wp-plugin-mojo' ), array( 'status' => rest_authorization_required_code() ) );
		}

...
export const SideNavMenu = () => {
	...
	const SubMenusManager = () => {
		...
		const subMenus = document.querySelectorAll('.wppm-app-navitem-submenu');
		subMenus.forEach((subMenu) => {
			subMenu.classList.add('nfd-hidden');
		});

		// open active's submenu if it exists
		const activeMenu = document.querySelector('.wppm-app-sidenav .active');
		if (activeMenu && null !== activeMenu.nextSibling && activeMenu.nextSibling.classList.contains('wppm-app-navitem-submenu')) {
	...
	return (
		<li className="nfd-mb-0">
			<NavLink
				...
				to={path}
				className={`wppm-app-navitem wppm-app-navitem-${label} nfd-flex nfd-items-center nfd-gap-3 nfd-px-3 nfd-py-2 nfd-rounded-md nfd-text-sm nfd-font-medium nfd-text-title leading-none hover:nfd-bg-slate-50 [&.active]:nfd-bg-[#E2E8F0]`}
			>
				{Icon &&
			...
			{subItems && subItems.length > 0 &&
				<ul className="wppm-app-navitem-submenu nfd-hidden nfd-ml-8 nfd-m-2 nfd-space-y-1.5">
	...
	return (
		<li className="nfd-m-0 nfd-pb-1">
			<NavLink
				onClick={(action && action instanceof Function) ? action : null}
				to={path} 
				className={`wppm-app-subnavitem wppm-app-subnavitem-${label} nfd-flex nfd-items-center nfd-gap-3 nfd-px-3 nfd-py-2 nfd-rounded-md nfd-text-sm nfd-font-medium nfd-text-body leading-none hover:nfd-bg-slate-50 [&.active]:nfd-bg-[#E2E8F0] [&.active]:nfd-text-title`}
			>
				{label}
			</NavLink>
		</li>
...
export const SideNav = () => {
	return (
		<aside className="wppm-app-sidenav nfd-shrink-0 nfd-hidden min-[783px]:nfd-block nfd-pb-6 nfd-bottom-0 nfd-w-56">
			<SidebarNavigation>
		...
		<header className="nfd-sticky nfd-z-30 nfd-top-0 min-[600px]:nfd-top-[46px] nfd-border-b nfd-border-line">
			<div className="nfd-flex nfd-justify-between nfd-items-center nfd-bg-white">
				...
				<Modal
					...
					className="wppm-app-sidenav-mobile nfd-z-40"
					initialFocus
				>
					<Modal.Panel className="nfd-p-0 nfd-overflow-visible">
						<div className="wppm-app-sidenav nfd-p-5 nfd-max-h-[70vh] nfd-overflow-y-auto">
							<SideNavMenu />
						</div>
					</Modal.Panel>
				</Modal>

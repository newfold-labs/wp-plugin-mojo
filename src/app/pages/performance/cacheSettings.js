...
const CacheSettings = () => {
	...
	const cacheOptions = [
		{
			label: (
				<span>
					<strong>{ __( 'Disabled', 'wp-plugin-mojo' ) }</strong>
					<span>
						{ __(
					...
					<em>{ __( 'Not recommended.', 'wp-plugin-mojo' ) }</em>
				</span>
			),
			value: 0,
		...
		},
		{
			label: (
				<span>
					<strong>{ __( 'Assets Only', 'wp-plugin-mojo' ) }</strong>
					<span>
						{ __(
		...
		},
		{
			label: (
				<span>
					<strong>
						{ __( 'Assets & Web Pages', 'wp-plugin-mojo' ) }
					</strong>
					<span>
						{ __(
	...
	useUpdateEffect( () => {
		mojoSettingsApiFetch( { cacheLevel }, setError, ( response ) => {
			setStore( {
				...store,
				cacheLevel,
			} );

...
const OldPostsComments = ({ setError, notify }) => {
	...
	const toggleDisableCommentsOldPosts = () => {
		mojoSettingsApiFetch({ disableCommentsOldPosts: !disableCommentsOldPosts }, setError, (response) => {
...
const CloseCommentsDays = ({ setError, notify }) => {
	...
	const closeCommentsDaysNoticeText = () => {
		...
		return (
			__('Comments on posts are disabled after ', 'wp-plugin-mojo') +
			closeCommentsDays +
	...
	const handleCloseCommentsDaysChange = (value) => {
		mojoSettingsApiFetch({ closeCommentsDays: value }, setError, (response) => {
...
const CommentsPerPage = ({ setError, notify }) => {
	...
	const commentsPerPageNoticeTitle = () => {
		return __('Comments setting saved.', 'wp-plugin-mojo');
	};
	...
	const handleCommentsPerPageChange = (value) => {
		mojoSettingsApiFetch({ commentsPerPage: value }, setError, (response) => {
	...
	return (
		<SelectField
			...
			label={__('Display ', 'wp-plugin-mojo') + commentsPerPage +
				__(' comments per page.', 'wp-plugin-mojo')}
...
const CommentSettings = () => {
	...
	return (
		<SectionSettings
			title={__('Comments', 'wp-plugin-mojo')}
			description={__('Comments allow visitors to provide feedback and respond to your posts or pages.', 'wp-plugin-mojo')}
		>

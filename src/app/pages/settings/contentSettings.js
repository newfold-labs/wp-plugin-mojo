...
const ContentRevisions = ({ setError, notify }) => {
	...
	const contentRevisionsNoticeText = () => {
		return (
			...
			contentRevisions +
			_n(' revision.', ' revisions.', parseInt(contentRevisions), 'wp-plugin-mojo')
		);
	};

	const contentRevisionsDescriptionText = () => {
		return (
			__('Saving drafts and updating published content creates revisions. Make changes with confidence, knowing you can take ', 'wp-plugin-mojo') +
			contentRevisions +
			_n(' step back.', ' steps back.', parseInt(contentRevisions), 'wp-plugin-mojo')
		);
	};

	const handleContentRevisionsChange = (value) => {
		mojoSettingsApiFetch({ contentRevisions: value }, setError, (response) => {
	...
	return (
		<SelectField
			...
			label={__('Number of revisions posts can save ', 'wp-plugin-mojo')}
...
const EmptyTrash = ({ setError, notify }) => {
	...
	const emptyTrashNoticeText = () => {
		return (
			...
			numTrashWeeks +
			_n( ' week.', ' weeks.', parseInt(numTrashWeeks), 'wp-plugin-mojo' )
		);
	};

	const handleEmptyTrashDaysChange = (value) => {
		mojoSettingsApiFetch({ emptyTrashDays: value }, setError, (response) => {
	...
	return (
		<SelectField
			...
			label={__('Trash emptying frequency ', 'wp-plugin-mojo')}
			description={
			__('The trash will automatically empty every ', 'wp-plugin-mojo') +
			numTrashWeeks +
			 _n( ' week.', ' weeks.', parseInt(numTrashWeeks), 'wp-plugin-mojo' )
...
const ContentSettings = () => {
	...
	return (
		<SectionSettings
			title={__('Content Options', 'wp-plugin-mojo')}
			description={__('Controls for content revisions and how often to empty the trash.', 'wp-plugin-mojo')}
		>

...
const ContentSettings = () => {
	...
	const emptyTrashDaysLabelText = () => {
		...
		return (
			<span>
				{ __( 'Empty trash every ', 'wp-plugin-mojo' ) }
				<strong>{ numTrashWeeks }</strong>
				{ _n( ' week.', ' weeks.', numTrashWeeks, 'wp-plugin-mojo' ) }
			</span>
		);
	};
	...
	useUpdateEffect( () => {
		mojoSettingsApiFetch( { contentRevisions }, setError, ( response ) => {
			setStore( {
				...store,
				contentRevisions,
			} );
	...
	useUpdateEffect( () => {
		...
		mojoSettingsApiFetch( { emptyTrashDays }, setError, ( response ) => {
			setStore( {
				...store,
				emptyTrashDays,
			} );
	...
	return (
		<Card className="card-content-settings">
			<CardHeader>
				<Heading level="3">
					{ __( 'Content Options', 'wp-plugin-mojo' ) }
				</Heading>
			</CardHeader>
